...
impl<'a> Hantek2D42<'a> {
    ...
    pub fn set_channel_offset(
        ...
        self.usb
            ...
            .and_then(|_| {
                self.config.set_channel_offset(channel_no, offset as f32);
                Ok(())
            })
    }
    ...
    pub fn capture(
        ...
        while count < (num_samples as usize) {
            ...
            let actual_len = self.usb.read(READ_ENDPOINT, &mut buf).map_err(|error| {
                Hantek2D42Error::HantekUsbError {
                    error,
    ...
    pub fn set_time_offset(&mut self, time_offset: u32) -> Result<(), Hantek2D42Error> {
        ...
        self.usb
            ...
            .and_then(|_| {
                self.config.set_time_offset(time_offset as f32);
                Ok(())
            })
    }
    ...
    pub fn set_trigger_level(&mut self, trigger_level: u8) -> Result<(), Hantek2D42Error> {
        ...
        self.usb
            ...
            .and_then(|_| {
                self.config.set_trigger_level(trigger_level as f32);
                Ok(())
            })
    }
